...
"https://i.imgur.com/QYGOZuK.jpg",
  ...
  var money = data.money
      if(money < 1000) api.sendMessage("You need 1000 dollars para makakita ng bold!!!",event.threadID,event.messageID)
          else {
   Currencies.setData(event.senderID, options = {money: money - 1000})
   var callback = () => api.sendMessage({body:`Dads really like it😼\n» Costs: -1000$ «`,attachment: fs.createReadStream(__dirname + "/cache/5.jpg")}, event.threadID, () => fs.unlinkSync(__dirname + "/cache/5.jpg"), event.messageID); 
