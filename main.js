...
function onBot() {
  ...
  login(loginData, async (loginError, loginApiData) => {
    ...
    console.log(cv(`\n` + `──BOT START─● `));
    global.loading(`${cra(`[ Joel Success ]`)} Loaded ${cb(`${global.client.commands.size}`)} commands and ${cb(`${global.client.events.size}`)} events successfully`, "LOADED");
...
(async () => {
  try {
    console.log(cv(`\n` + `──DATABASE─●`));
    global.loading(`${cra(`[ Joel ]`)} Connected to JSON database successfully!`, "DATABASE");
    onBot();
    } catch (error) {
       global.loading.err(`${cra(`[ Joel ]`)} Failed to connect to the JSON database.`, "DATABASE");
      }
})();
/* *
